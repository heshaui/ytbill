<template>
    <div v-if="item.meta.sidebar !== false">
        <!-- 注视 @click="navigate" -->
        <router-link v-if="!hasChildren" v-slot="{ href, isActive, isExactActive }" custom :to="item.query ? {path: resolvePath(item.path), query: item.query} : resolvePath(item.path)">
            <a :href="isExternal(resolvePath(item.path)) ? resolvePath(item.path) : href" :class="[isActive && 'router-link-active', isExactActive && 'router-link-exact-active']" :target="isExternal(resolvePath(item.path)) ? '_self' : '_self'">
        ...
        <el-submenu v-else :title="item.meta.title" :index="resolvePath(item.path)">
            ...
            <SidebarItem v-for="route in item.children" :key="route.path" :item="route" :base-path="route.query ? {path: resolvePath(item.path), query: route.query} : resolvePath(item.path)" />
        </el-submenu>
    </div>
</template>
