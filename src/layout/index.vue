                        ...
                        >
                            <transition-group name="sidebar">
                                <template v-for="route in $store.getters['menu/sidebarRoutes']">
                                    <!-- id=170是班长座席的菜单 -->
                                    <SidebarItem v-if="(route.meta.sidebar !== false && route.id != 170) || (route.id == 170 && isMonitor)" :id="route.id" :key="route.path" :item="route" :base-path="route.path" />
                                </template>
                            </transition-group>
                        </el-menu>
                    </div>
...
let versionConfig = require('../version.json')
export default {
    name: 'Layout',
    components: {
        Logo,
    ...
    data() {
        return {
            version: versionConfig.version, // 版本号
            isRouterAlive: true,
            routePath: '',
            sidebarScrollTop: 0,
            scrollTop: 0,
        ...
        realSidebarWidth() {
            ...
            if (this.$store.state.settings.mode == 'pc') {
                ...
                if (this.$store.state.settings.sidebarCollapse) {
                    ...
                    if (this.$store.state.settings.screenW == 'sc') {
                        realSidebarWidth += 180
                    } else {
    ...
    watch: {
        ...
        '$store.state.commonStatus.remindData': {
            handler() {
                this.showRemind = true
                this.remindData = local.getItem('remindData')
            }
        }
    },
    mounted() {
    ...
    methods: {
        ...
        onSidebarScroll: debounce(function(e) {
            this.sidebarScrollTop = e.target.scrollTop
            local.setItem('sidebarScrollTop', this.sidebarScrollTop)
        }, 500),
        onScroll() {
